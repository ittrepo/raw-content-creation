...
# Config
INPUT_FILE = Path("hotelbeds_new_2.txt")
    ...
    while attempt < MAX_RETRIES:
        try:
            ...
            if resp.ok:
                ...
                return (hotel_id, "saved", resp.status_code)
            else:
                if 400 <= resp.status_code < 500:
                    return (hotel_id, "error", resp.status_code, f"Client error: {resp.text[:200]}")
                attempt += 1
                time.sleep(backoff)
            ...
            time.sleep(backoff)
            backoff *= RETRY_BACKOFF
    return (hotel_id, "error", "N/A", f"Failed after {MAX_RETRIES} attempts. Last exception: {last_exc}")
...
def main():
    ...
    with ThreadPoolExecutor(max_workers=WORKERS) as executor:
        future_to_id = {executor.submit(fetch_and_save, hid): hid for hid in ids}
        for i, future in enumerate(as_completed(future_to_id), start=1):
            result = future.result()
            summary.append(result)
            hid = result[0]
            if result[1] == "saved":
                print(f"[{i}/{len(ids)}] {hid} saved (HTTP {result[2]}).")
            else:
                print(f"[{i}/{len(ids)}] {hid} error -> {result[3]}")
